	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "regist" :
            ...
            */
			...
			//.jsp는 그 부분만 등록되는 것이지만 전체 리스트가 다 등록되어야 하기
			// 때문에 board/board_write.jsp이 아닌, /JspBasic/list.board라고 등록해야 함
		...
		case "list":
			...
			/*
			 ...
			 request 객체에 list를 담아서 전달하려 하는데, 
			 sendRedirect를 사용하면 응답이 바로 나가면서 
			 기존의 request 객체가 소멸해 버립니다. 
			 여기서는 forward방식을 사용해서 request를 원하는 
			 jsp 파일로 전달해서 그쪽에서 응답이 나갈 수 있도록 처리해야 
			 합니다.
			 */
//			response.sendRedirect("board/board_list.jsp");
			
			//request 객체를 다음 화면까지 운반하기 위한 forward 기능을 제공하는 
			//객체
			//-> RequestDispatcher
		...
		case "content":
			...
			dp.forward(request, response);
			break;
//			RequestDispatcher dp2 = request.getRequestDispatcher("board/board_content.jsp");
		...
		case "delete" : 
			...
			response.sendRedirect("/JspBasic/list_board.jsp");
			break;
